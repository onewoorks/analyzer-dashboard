...
const sortingResult = (unsortdata) => {
    ...
    Object.keys(unsortdata).map((value, key) => {
        console.log(unsortdata[key])
        var data = {
            key: key,
            field: unsortdata[key]['test_name'],
            value: unsortdata[key]['measurement_value'],
        }
        sort_data.push(data)
        return sort_data
    })
...
export default class Sysmex extends Component {
    constructor(props) {
        super(props)
        this.state = {
        ...
        return (
            <React.Fragment>
                <Row gutter={16}>
                    ...
                    <Col span={16}>
                        <Card title="Sysmex XN-350 & KX 21 Result" bordered={true} headStyle={{ backgroundColor: '#f2f4f5' }}>
                            {this.result_slip()}
                            <Table columns={columns} dataSource={table_data} pagination={false}
                                size="small" />
                            <br />

                            <Link to='/sysmex-xn-350-kx-21'>
